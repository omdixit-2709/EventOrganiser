...
router.get('/google/callback',
    (req, res, next) => {
        ...
        passport.authenticate('google', { 
            failureRedirect: 'https://event-organiser-seven.vercel.app/login?error=auth_failed' || 'http://localhost:3000/login?error=auth_failed',
            session: true 
        })(req, res, next);
    },
    (req, res) => {
        ...
        res.redirect('https://event-organiser-seven.vercel.app/auth/callback' || 'http://localhost:3000/auth/callback' );
    }
);
...
router.post('/refresh-token', isAuthenticated, async (req, res) => {
    try {
        ...
        const oauth2Client = new google.auth.OAuth2(
            ...
            'https://event-organiser-seven.vercel.app/auth/callback' || 'http://localhost:3000/auth/callback'
        );
...
router.get('/logout', (req, res) => {
    ...
    req.logout((err) => {
        ...
        req.session.destroy((err) => {
            ...
            res.redirect('https://event-organiser-seven.vercel.app/login' || 'http://localhost:3000/login');
        });
    });
});
